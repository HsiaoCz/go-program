...
select ename from dept where sal is not null;

and 并且 
select empno from dept where empno=7566 and sal=2975.00;
or 或者
select sal from dept where ename='JSONS' or ename='FORD' ;
and和or同时出现的话，有优先级问题吗？
and和or同时出现，and优先级较高。如果想让or先执行，需要加“小括号”.以后在开发中，如果不确定优先级，就加小括号就行了。

in 包含，相当于多个 or （not in 不在这个范围中）
select * from dept where sal in (2400,3000)
...
//上面那个会出错，因为sum(sal)需要先分组，
//修改
select sum(sal),deptno from emp group by deptno having sum(sal)>9000 order by deptno asc;
//having和where 差不多，但是不能单独出现，必须和group by 一起使用
